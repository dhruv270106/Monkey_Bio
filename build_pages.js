...
<head>
    ...
    <script>
      ...
      window.supabaseClient = (window.supabase || window.supabasejs).createClient(supabaseUrl, supabaseKey);
    </script>
    <style>
      ...
      <div class="min-h-screen flex bg-[#F5F7FA]">
         <div class="w-full lg:w-1/2 flex items-center justify-center p-8 sm:p-12 lg:p-24 bg-white relative z-10 rounded-r-[40px] shadow-2xl overflow-hidden">
            <div class="w-full max-w-md animate-fade-in opacity-0" style="animation-delay: 0.2s;">
              ...
              <div class="space-y-4">
                 <button onclick="window.supabaseClient.auth.signInWithOAuth({provider: 'google', options: { queryParams: { prompt: 'select_account' }, redirectTo: window.location.origin + '/onboarding.html' }})" class="w-full py-3.5 px-4 rounded-xl font-bold bg-[#F5F7FA] text-secondary hover:bg-[#EAEFF4] transition-colors flex items-center justify-center gap-3">
            ...
            document.getElementById('authForm').addEventListener('submit', async (e) => {
              ...
              try {
                if (isLogin) {
                  ...
                  if (authData.session && authData.user && username) {
