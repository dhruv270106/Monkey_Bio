...
export default function InsightsSection({ profile }: InsightsSectionProps) {
  ...
  if (loading) {
    return (
      <div className="flex-1 flex items-center justify-center bg-white text-primary">
    ...
    <div className="flex-1 overflow-y-auto p-12 bg-white no-scrollbar">
      <div className="max-w-5xl mx-auto space-y-12">
        ...
        <div className="bg-gray-50 rounded-[40px] p-10 border border-gray-100 flex flex-col gap-8 h-[400px]">
          ...
          <div className="flex-1 flex items-end justify-between gap-4 px-4 pb-4 h-full overflow-hidden">
             {chartData.map((d, i) => (
               <div key={i} className="flex-1 group relative flex items-end gap-1 h-full">
                 <motion.div 
                   initial={{ height: 0 }} 
                   animate={{ height: `${(d.views/maxVal) * 100}%` }} 
                   className="flex-1 bg-primary/30 rounded-t-lg group-hover:bg-primary transition-colors relative"
                 >
                   <div className="absolute -top-10 left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-100 bg-secondary text-white text-[8px] font-black px-2 py-1.5 rounded-lg transition-opacity whitespace-nowrap z-10 shadow-xl">
                     {d.views} Views
                   </div>
                 </motion.div>
                 <motion.div 
                   ...
                   className="flex-1 bg-blue-500/30 rounded-t-lg group-hover:bg-blue-500 transition-colors relative"
                 >
                   <div className="absolute -top-10 left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-100 bg-secondary text-white text-[8px] font-black px-2 py-1.5 rounded-lg transition-opacity whitespace-nowrap z-10 shadow-xl">
                     {d.clicks} Clicks
                   </div>
          ...
          <div className="bg-white rounded-[40px] p-8 border border-gray-100 shadow-sm flex flex-col gap-6">
            ...
            <div className="space-y-4">
              {topLinks.map((link, i) => {
                ...
                return (
                  <div key={i} className="flex items-center justify-between group p-2 hover:bg-gray-50 rounded-2xl transition-all">
                    <div className="flex items-center gap-4">
                      <div className="w-10 h-10 rounded-xl bg-gray-50 flex items-center justify-center shrink-0">
                        <i className={`fi ${appConfig?.icon || 'fi-rr-link'} ${appConfig?.color || 'text-gray-400'}`}></i>
                      </div>
                      <div className="min-w-0">
                        <h4 className="text-sm font-black text-secondary truncate">{link.title}</h4>
              ...
              {topLinks.length === 0 && <p className="text-xs text-gray-400 font-bold text-center py-10 italic">No link clicks tracked yet</p>}
            </div>
          </div>
