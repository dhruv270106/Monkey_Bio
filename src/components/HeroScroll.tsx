...
const ShowcaseCard = ({ data, isMobile }: { data: typeof CREATOR_DATA[0], isMobile: boolean }) => {
  return (
    <div className={`${isMobile ? 'w-[320px] h-[320px] rounded-[32px]' : 'w-[420px] h-[520px] rounded-[60px]'} overflow-hidden relative group bg-white shadow-2xl shrink-0`}>
      {/* BACKGROUND IMAGE */}
      <img src={data.img} alt={data.name} className="w-full h-full object-cover" />
      
      {/* TEXT OVERLAY BOTTOM */}
      <div className={`absolute inset-x-0 bottom-0 ${isMobile ? 'h-24 p-5' : 'h-40 p-8'} bg-gradient-to-t from-black/80 via-black/20 to-transparent z-10 flex flex-col justify-end`}>
         <p className={`text-white font-black ${isMobile ? 'text-base' : 'text-lg md:text-xl'} drop-shadow-2xl leading-tight`}>
           {data.name}
         </p>
         <p className="text-white/80 font-bold text-[9px] uppercase tracking-widest mt-0.5">
           {data.role}
         </p>
      </div>
      
      {/* FLOATING UI */}
      <div className={`absolute ${isMobile ? 'right-3 top-3 w-[100px] p-2.5' : 'right-6 top-10 w-[150px] p-4'} bg-white/95 rounded-[20px] shadow-2xl flex flex-col items-center border border-white/40 backdrop-blur-sm z-20`}>
         ...
         <p className="font-extrabold text-[8px] text-gray-400 mb-1.5 tracking-tighter uppercase">{data.username}</p>
         <div className="w-full space-y-1">
           {data.links.map((l, i) => (
             <div key={i} className={`w-full ${isMobile ? 'h-4 text-[6px]' : 'h-6 text-[6px]'} rounded-md bg-gray-50 flex items-center justify-center font-bold text-linktree-text`}>
               {l}
             </div>
           ))}
         </div>
...
export default function HeroScroll() {
  ...
  if (isMobile) {
    return (
      <div className="w-full pt-4 pb-20 overflow-visible relative select-none z-10 pointer-events-none">
        <motion.div 
          animate={{ x: [0, -1200] }} 
          transition={{ 
            x: {
